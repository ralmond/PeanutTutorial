...
heart.features <- list(
  ...
  CP=c(TypicalAnginal=1, AtypicalAnginal=2, NonAnginal=3, Asymptotic=4),
  trestbps=c(0,low=120,moderate=140,high=300),
  chol=c(0,Normal=200, Moderate=250, High=300, VeryHigh=500),
  ...
  exang=c(Yes=1, No=0),
  oldpeak=c(-1,Zero=1,Low=2,High=10),
...
Netica supports _case files_, which are essentially CSV files.  These are easier ways to manage large numbers of cases.
...
Note that good programming style requires that the `CaseStream` be closed when you are done with it.  To ensure this, use `WithOpenCaseStream(`_stream_,`{...})`.
...
Note that Netica uses a different random number sequence than R.  To make 
a reproducible sequence, we need to create a `NeticaRNG` object and give it a seed.  Use the `FreeNeticaRNG` method to free it when you are done (or use `WithNeticaRNG` to free it at the end of the computation.
...
Note that continuous variables are left at their numeric values.  Need
to reread cleveland data and not do the conversion this time.
...
`LearnFindings`, `LearnCases`, `LearnCPTs`

## Learning Structure from Data

## Validating inferences using a second data set.
