...
$$W(H:E) = 
\log \frac{\Pr(E|H)}{\Pr(E|\overline{H})} =
\log \frac{\Pr(H|E)}{\Pr(\overline{H}|E)} - \log \frac{\Pr(H)}{\Pr(\overline{H})}$$
...
$$W(H:E_2|E_1) = 
\log \frac{\Pr(E_2|H,E_1)}{\Pr(E_2|\overline{H},E_1)}$$
...
When choosing next "test" (task/item) look at expected value of WOE where expectation is taken wrt $\Pr(E|H)$ .

where represent the possible results.

$$EW(H:E) = \sum_{k=1}^K W(H:e_k)\Pr(e_k|H)$$
...
Calculate for each candidate item $\Pr(E_i|H)$ and $\Pr(E_i|\overline{H})$
...
$\textrm{VoI}(T) = E_T \left [
\max_d E_{\boldsymbol{S}} u(d,\boldSymbol{S}) -
\max_d E_{\boldsymbol{S}|T} u(d,\boldSymbol{S})
...
$$\sum_x \Pr(x) \sum_y \Pr(y|x) \log \frac{\Pr(y|x)}{\Pr(y)}$$
