...
[Download the mini-ACED file](miniACED.zip)
Unzip it in `miniACED` and that this folder is in the same directory as this `qmd` file. 

Next, change directory into that directory.
```{r cdMiniAced}
setwd("miniACED")
```
...
EMnet <- ReadNetworks(paste(EMtable[t.name,"EM"],"dne", 
                            sep="."),
                      session=sess)
obs <- AdjoinNetwork(Fred.SM,EMnet)
